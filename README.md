...
| `ANSIBLE_UI_REMOTE_USER` | **Optional:** sets Ansible’s default SSH user (`ANSIBLE_REMOTE_USER`). Use when the service runs as a dedicated account (e.g. `ansible-ui`) but targets should be reached as `root` or `ubuntu`. Inventory `ansible_user` per host still overrides. If unset, jobs that use an **SSH key or password credential** default to **`root`** when credential Extra does not set `ansible_user` (same default as **Deploy public key**). |
...
**SSH from the server (systemd):** Jobs run as the service user (`ansible-ui`), not as you on the shell. Without configuration, Ansible would try to SSH as **`ansible-ui`**, which causes *Permission denied (publickey)* if you deployed keys to **`root`** (the deployer’s default). The runner sets **`ANSIBLE_REMOTE_USER=root`** when you use an SSH key or password credential and omit **`ansible_user`** in credential Extra — matching **Deploy public key**. Override per host in inventory, or set **`ansible_user: ubuntu`** (etc.) in **Credentials → Extra**, or **`ANSIBLE_UI_REMOTE_USER`** / **`ANSIBLE_REMOTE_USER`** on the service. For **SSH password** credentials, install **`sshpass`** on the server (`apt install sshpass`) so Ansible can log in non-interactively. The runner normalizes inventory, credential extra, and job-template extra vars (CRLF, BOM, stray spaces, zero-width/bidi Unicode) so pasted Windows/Web content does not produce errors like *hostname contains invalid characters*.
